...
**/
  ...
  else if (RootPtr->Key != K) // klic neni hledanym 
  {  
    if (RootPtr->Key > K) return (BSTSearch(RootPtr->LPtr, K, Content)); // aktualni klic vetsi nez hledany => doleva 
    else return (BSTSearch(RootPtr -> RPtr, K, Content)); // mensi => doprava
  } 
  else // klic roven hledanemu => nasel jsem
  {
...
**/
  ...
  if (!(*RootPtr)) 
  { 
    tBSTNodePtr knot = malloc(sizeof(struct tBSTNode)); 
	
    if (!(knot)) return;
    else
    {
      knot->Key = K; 
      knot->BSTNodeCont = Content; 
      knot->LPtr = NULL; 
      knot->RPtr = NULL;
      *RootPtr = knot; 
    }
  } 
  else if (K < (*RootPtr)->Key) BSTInsert(&((*RootPtr)->LPtr), K, Content); // klic je mensi => vkladam doleva
  else if (K > (*RootPtr)->Key) BSTInsert(&((*RootPtr)->RPtr), K, Content); // klic vetsi => vkladam doprava 
  else (*RootPtr) -> BSTNodeCont = Content; // kdyz uz se nemam kam norit, nalezeno, vlozim obsah
}
...
**/
	if((!RootPtr) || (!(*RootPtr))) return;
  else
  {
    if ((*RootPtr)->RPtr) ReplaceByRightmost(PtrReplaced, &((*RootPtr) -> RPtr));      
    else 
    { 
      PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont; // nahrazeni obsahu
      PtrReplaced->Key = (*RootPtr)->Key; // a klice
      tBSTNodePtr tmp = (*RootPtr); // zaloha korene
      (*RootPtr) = (*RootPtr)->LPtr; // novy koren
      free(tmp); // uvolneni stareho korene
    }
    return; 
  }  	
}
...
**/
  ...
  { 
    if (K < (*RootPtr)->Key) BSTDelete(&((*RootPtr)->LPtr), K); // klic mensi => doleva
    else if (K > (*RootPtr)->Key) BSTDelete(&((*RootPtr)->RPtr), K); // klic vetsi => doprava 
    else 
    {
      tBSTNodePtr ptr = *RootPtr; // zaloha

      if (!(ptr -> RPtr)) 
      { 
        (*RootPtr) = ptr->LPtr; // koren vpravo nic nema => bude levy
        free(ptr);
      } 
      else 
      {
        if (!(ptr->LPtr)) 
        { 
          (*RootPtr) = ptr->RPtr; // koren vlevo nic nema => bude pravy
          free(ptr);
        }
...
**/
  ...
  { 
    BSTDispose(&(*RootPtr)->RPtr); // dispose doprava
    BSTDispose(&(*RootPtr)->LPtr); // dispose doleva
    free(*RootPtr); // uvolnit
    *RootPtr = NULL; // oznacit za prazdny
    return;
  }
}
