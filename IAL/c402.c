...
**/
  ...
  if (!(*RootPtr))
  {
    if (!(ptr = malloc(sizeof(struct tBTNode)))) return; 
    else
    {
      ptr->Cont = Content;
      ptr->LPtr = NULL;
  ...
  else
  {
    ...
    do //prochazime strom dokud nenalezneme poradovanou pozici
    {
      if ((ptr->Cont) == Content) return; 
      else if((ptr->Cont) < Content) // vkladana hodnota vetsi => jdeme vpravo
      {
        father = ptr;
        ptr = ptr->RPtr;
      }
      else // mensi => jdeme vlevo
      {
        father = ptr;
        ptr = ptr->LPtr;
      }
    ...
    else
    {
      ...
      if ((father->Cont) > Content) father->LPtr = ptr; // navazani zleva
      else father->RPtr = ptr; // navazani zprava  
    }
  } 
}
...
**/
  ...
  else
  {
    ...
    while(!(SEmptyP(&stackP))) //dokud neprojdeme vsechny uzly
    {
      ptr = STopPopP(&stackP);
      SPushP(&stackP, ptr);
...
**/
  ...
  else
  {
    ...
    do
    {
      if(*RootPtr)  
      {
        if(!((*RootPtr)->RPtr)) SPushP(&stackP, (*RootPtr)->RPtr);  // zapsani uzlu do zasobniku

        ptr = (*RootPtr);
        (*RootPtr) = (*RootPtr)->LPtr;  // posun na dalsi uzel
        free(ptr); 
      }
      else if(!SEmptyP(&stackP)) (*RootPtr) = STopPopP(&stackP);
      else return;
    } while((*RootPtr) || (!(SEmptyP(&stackP))));
  }
}
